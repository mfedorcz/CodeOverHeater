...
--   if not data.lit or data.lit == "" then
...
--   end
  ...
  data.accidents = way:get_value_by_key("accidents")

  if data.accidents ~= nil then
    data.accidents = tonumber(data.accidents)
    if data.accidents > 3 then
      result.forward_mode = mode.inaccessible
      result.backward_mode = mode.inaccessible
      return
    end
  ...
  -- maxspeed
  -- TODO adjusting max speed?
  limit( result, data.maxspeed, data.maxspeed_forward, data.maxspeed_backward )

  local max_allowed = profile.properties.max_allowed_speed
  if (data.maxspeed and data.maxspeed > max_allowed) or
      (data.maxspeed_forward and data.maxspeed_forward > max_allowed) or
      (data.maxspeed_backward and data.maxspeed_backward > max_allowed) then
...
function safety_handler(profile,way,result,data)
  ...
  if profile.properties.weight_name == 'cyclability' then
    ...
    -- TODO traffic volume
    data.traffic_volume = way:get_value_by_key("traffic_volume")
    data.name = way:get_value_by_key("name")
    if data.traffic_volume ~= nil then
      
      print("BEFORE", result.weight)
      result.weight = result.weight + data.traffic_volume
      print("TRAFFIC VOLUME", data.traffic_volume, "Name", data.name, "Weight", result.weight)
    end
